"""Timewarp flow base classes adapted for PT swap flows.

This module provides the core flow abstractions from Timewarp adapted for
...
Key adaptations from original Timewarp:
...
class PTFlow(nn.Module, abc.ABC):
    """Base class for PT swap flow layers adapted from Timewarp Flow.
    
    Key differences from Timewarp:
...
class PTSequentialFlow(PTFlow):
    """Sequential container for PT flow layers adapted from Timewarp ConditionalSequentialFlow.
...
class PTNVPCouplingLayer(PTFlow, abc.ABC):
    """Base NVP coupling layer for PT flows adapted from Timewarp NVPCouplingLayer.
